...
class Transcriber:
    ...
    def __init__(self, audio_path, audio_format, api_key):
        self.audio_path = audio_path
        self.audio_format = audio_format
        self.client = genai.Client(api_key=api_key)
        self.file = self.client.files.upload(path=audio_path)
    ...
    async def transcribe(self, language):
        system_instruction = """
        ...
        return await self.__run_prompt_async(
            [
                types.Part.from_uri(file_uri=self.file.uri, mime_type=f"audio/{self.audio_format}"),
                language,
            ],
            system_instruction
        )
    ...
    def __run_prompt(self, contents, system_instruction):
    ...
    async def __run_prompt_async(self, contents, system_instruction):
